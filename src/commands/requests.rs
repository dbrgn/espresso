...
use crate::commands::responses::{self, EmptyResponse, WifiModeResp};
use crate::types;
...
/// Restarts the module.
#[derive(Debug, AtatCmd)]
#[at_cmd("+RST", EmptyResponse)]
pub struct Restart;

/// Queries the current WiFi mode.
#[derive(Debug, AtatCmd)]
#[at_cmd("+CWMODE_CUR?", WifiModeResp)]
pub struct GetCurrentWifiMode;

/// Queries the default WiFi mode.
#[derive(Debug, AtatCmd)]
#[at_cmd("+CWMODE_DEF?", WifiModeResp)]
pub struct GetDefaultWifiMode;

/// Sets the current WiFi mode.
#[derive(Debug, AtatCmd)]
#[at_cmd("+CWMODE_CUR", EmptyResponse)]
pub struct SetCurrentWifiMode {
    #[at_arg(position = 0)]
    pub mode: types::WifiMode,
}

/// Sets the default  WiFi mode.
///
/// The configuration is persisted to flash.
#[derive(Debug, AtatCmd)]
#[at_cmd("+CWMODE_DEF", EmptyResponse)]
pub struct SetDefaultWifiMode {
    #[at_arg(position = 0)]
    pub mode: types::WifiMode,
}
