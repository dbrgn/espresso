...
use atat::{AtatClient, ClientBuilder, Clock, DefaultDigester, DefaultUrcMatcher, GenericError, Queues};
    ...
    CLK: Clock<TIMER_HZ>,
{
    ...
    pub fn new(
        ...
        let (client, ingress) = ClientBuilder::new(serial_tx, clock, config).build(queues);
        (Self { client }, ingress)
    }
